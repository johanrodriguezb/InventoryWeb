...
$consu = "SELECT Ambiente FROM activos where Ambiente='" . $I . "'";
$data = mysqli_query($con, $consu);
$sql = mysqli_fetch_array($data);

$pdf->cell(0, 0, 'Activos Que Se Encuentran En El Ambiente: ' . $sql["Ambiente"] . '', 0, 0, 'S');
...
$pdf->cell(20, 6, "Ambiente", 1, 1, 'C', 1);
...
while ($row = mysqli_fetch_array($datos)) {
    ...
    $pdf->SetFillColor(251, 255, 1);
    $pdf->Cell(20, 6, $row["Ambiente"], 1, 1, 1, 'C');
}

$pdf->Ln(10);
$consu = "SELECT COUNT(Ambiente) as Atotal from activos where Ambiente = '" . $I . "'";
$data = mysqli_query($con, $consu);
$sql = mysqli_fetch_array($data);

$pdf->cell(0, 0, 'Total de Activos: ' . $sql["Atotal"] . '');
